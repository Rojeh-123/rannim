document.addEventListener('DOMContentLoaded', function () {
  ...
  function updatePlayAllIcon(playing) {
    ...
    playAllBtn.src = playing
      ? '<?php echo base_url(); ?>dashboard/assets/imgs/pause-fill.png'
      : '<?php echo base_url(); ?>dashboard/assets/imgs/play-fill.png';
  }
  ...
  function playNextFromQueue(prevIndex) {
    ...
    const nextPos = pos + 1;
    if (nextPos < playQueue.length) {
      startPlayingIndex(playQueue[nextPos]);
    } else {
      isPlayingAll = false;
  ...
  songs.forEach((s, idx) => {
    ...
    sh.addEventListener('click', async () => {
      try {
        if (navigator.share) {
          await navigator.share({ title: s.title, text: s.title + '\n' + s.path });
        } else if (navigator.clipboard) {
          await navigator.clipboard.writeText(s.path);
          alert('Song URL/path copied to clipboard');
        } else {
  ...
  if (followToggle) {
    followToggle.addEventListener('click', () => {
      ...
      followToggle.classList.toggle('unfollow-style', isFollowing);
    });
  }
  ...
  if (shareAllBtn) {
    shareAllBtn.addEventListener('click', async () => {
      ...
      try {
        if (navigator.share) {
          await navigator.share({ title: 'Songs', text });
        } else if (navigator.clipboard) {
